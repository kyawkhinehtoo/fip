                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Received Amount</th>
    ...
    setup(props){
      ...
      const form = reactive({
        ...
        function submit() {
           if(! editMode.value){
            form._method = 'POST';
            Inertia.post('/PaymentRecord', form, {
            preserveState: true,
            onSuccess: (page) => {
                closeModal()
                resetForm()
            ...
            onError: (errors) => {
                ...
                Inertia.post('/PaymentRecord/' + form.id, form,{
                onSuccess: (page) => {
                closeModal()
                resetForm()
                Toast.fire(
        ...
        function deleteRow(data) {
                ...
                data._method = 'DELETE';
                Inertia.post('/PaymentRecord/' + data.id, data)
                closeModal()
                resetForm()
        }
        function openModal() {
        ...
        const searchTsp = () =>{
          ...
          Inertia.get('/PaymentRecord/', {township : search.value}, { preserveState: true })
        }
