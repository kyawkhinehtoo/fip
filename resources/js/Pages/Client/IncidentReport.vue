    ...
    <div class="py-2">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6 w-full">
            ...
            <div class="py-2 col-span-1 sm:col-span-1">
              ...
              <div class="mt-1 flex rounded-md shadow-sm">
                ...
                <litepie-datepicker
                        placeholder="Please choose Ticket Range"
                        :formatter="formatter"
                        separator=" to "
                        v-model="form.period" tabindex="3"></litepie-datepicker>
              </div>
            </div>
      ...
      <div v-if="incidents" class="max-w-full mx-auto sm:px-6 lg:px-8 mt-4">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ticket Type</th>
                <th scope="col" class="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ticket Start Date</th>
                <th scope="col" class="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ticket Close Date</th>
        ...
        <span v-if="incidents.total" class="w-full block mt-4">
          <label class="text-xs text-gray-600">{{ incidents.data.length }} Invoices in Current Page. Total Number of Invoices : {{ incidents.total }}</label>
        </span>
...
const showTime =(mins)=>{
    ...
    const doExcel = () => {
         axios.post("/exportIncidentReportExcel", 
          form)
          .then(response => {
