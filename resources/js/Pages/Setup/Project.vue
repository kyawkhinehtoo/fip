                  ...
                  <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="">
                      <div class="mb-4">
                        ...
                        <input type="text" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" id="name" placeholder="Enter Project Name" v-model="form.name" />
  ...
  setup(props) {
    ...
    function submit() {
      if (!editMode.value) {
        ...
        form._method = "PUT";
        Inertia.post("/project/" + form.id, form, {
          onSuccess: (page) => {
            closeModal();
            resetForm();
            Toast.fire({
    ...
    function deleteRow(data) {
      ...
      data._method = "DELETE";
      Inertia.post("/project/" + data.id, data);
      closeModal();
      resetForm();
    }
    function openModal() {
    ...
    const searchTsp = () => {
      ...
      Inertia.get("/project/", { project: search.value }, { preserveState: true });
    };
